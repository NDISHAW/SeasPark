...
app.post('/api/careers', async (req, res) => {
  const { title, descriptions, requirements, location ,address,category,experience} = req.body;
    try {
      const newJobOpening = await knexInstance("job_openings")
        .insert({ title, descriptions, requirements, location ,address,category,experience})
        .returning("*");
