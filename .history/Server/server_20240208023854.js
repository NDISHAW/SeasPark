...
app.post("/sendEmail", upload.single("pdfFile"), async (req, res) => {
  try {
    const { email, fullName, phoneNumber, jobNumber, message, title,id} =
      req.body;
    ...
    const mailOptions = {
      ...
      subject: "Job Application for ${dv}",
      html: `
        <h3>Job title: ${title} Job id ${jobNumber}</h3>
        <p><span>Full Names: ${fullName} </span> <span>phoneNumber: ${phoneNumber} </span> </p>
        <p>${message}</p>
