...
async function createJobOpeningsTable() {
  ...
  try {
    ...
    console.error("Error creating job openings table:", error);
  }
}
...
app.get("/api/careers", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM job_openings");
...
app.post("/api/careers", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO job_openings (title, description) VALUES ($1, $2) RETURNING *",
      [title, description]
    );
...
app.put("/api/careers/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE job_openings SET title = $1, description = $2 WHERE id = $3 RETURNING *",
...
app.delete("/api/careers/:id", async (req, res) => {
  const id = parseInt(req.params.id);
  try {
    const result = await pool.query("DELETE FROM job_openings WHERE id = $1", [id]);
    if (result.rowCount === 0) {
      res.status(404).json({ error: "Career not found" });
    } else {
